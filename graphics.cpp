...
int SDL_main(int argc, char* argv[]){
    ...
    createObj(objects, {190, -150, 190}, {0, 0, 255}, 10, 0, true, true, true, true);
    ...
    while (run) {
        ...
        // Let the user control when to stop the rotation
        if (doRotation){
            rotation.x += stof(argv[2]);
            rotation.y += stof(argv[3]);
            rotation.z += stof(argv[4]);
        }
